    ...
    for platform, domen, sobachka in zip(social_media_platforms, domens, sobachka):
        ...
        try:
            ...
            if response.status_code == 200:
                print(f"{Colors.red}Valid: {url}{Colors.reset}")
            else:
    ...
    for network, links in results.items():
        ...
        for link in links:
            if "youtube.com" in link or "tiktok.com" in link or "instagram.com" in link or "facebook.com" in link or "pinterest.com" in link or "snapchat.com" in link or "linkedin.com" in link or "telegram.me" in link:
                print(f"{Colors.red}Valid: {link}{Colors.reset}")
            else:
...
def generate_proxies(num_proxies):
    ...
    protocol_choice = input(Colorate.Horizontal(Colors.white_to_red, "Enter the protocol (http, socks4, socks5): ")).lower()
...
def theme():
    ...
    if choice == "1":
        file_path = "main.py"  # Replace with the actual path to your mainred.py file
        subprocess.run(["python", file_path])  # Use `python` command to execute the file
    elif choice == "2":
        file_path = "maingreen.py"  # Replace with the actual path to your maingreen.py file
        subprocess.run(["python", file_path])  # Use `python` command to execute the file
    else:
...
'''))
        choice = input(Colorate.Horizontal(Colors.white_to_red, '''
                                                    @User/SLACK/main.py> $'''))
        print()
        if choice == "1":
            phone_number = input(Colorate.Horizontal(Colors.white_to_red, "Enter a phone number (+79999999999): "))
            ...
            ip = input(Colorate.Horizontal(Colors.white_to_red, "Enter an IP address: "))
            if ip == "" or ip == " " or ip == "7":
                print("IP is empty")
            else:
                result = search_ip(ip)
...
7.https://t.me/redan_search_bot
8.https://t.me/gta_searchers_bot
9.https://t.me/funstat_kbot
10.https://t.me/usinfobot''')
        elif choice == "6":
            url = input(Colorate.Horizontal(Colors.white_to_red, "Enter the URL to attack: "))
            if url == "" or url == " " or url == "6":
                print("URL is empty")
            else:
                number_of_threads = int(input(Colorate.Horizontal(Colors.white_to_red, "Enter the number of threads: ")))
                duration = int(input(Colorate.Horizontal(Colors.white_to_red, "Enter the duration of the attack (in seconds): ")))
                ddos_attack(url, number_of_threads, duration)
        elif choice == "5":
            ip = input(Colorate.Horizontal(Colors.white_to_red, "Enter the IP address to attack: "))
            if ip == "" or ip == " " or ip == "6":
                print("IP is empty")
            else:
                number_of_threads = int(input(Colorate.Horizontal(Colors.white_to_red, "Enter the number of threads: ")))
                ...
                nickname = input(Colorate.Horizontal(Colors.white_to_red, "Enter a nickname to search: "))
                osint_search_google(nickname)
        elif choice == "14":
                nickname = input(Colorate.Horizontal(Colors.white_to_red, "Enter a nickname to search: "))
                osint_search_nickname(nickname)
        elif choice == "2":
            prefix = input(Colorate.Horizontal(Colors.white_to_red, "Enter the prefix of the phone number (e.g., 999): "))
            if prefix == "" or prefix == " " or prefix == "6":
                print("Prefix is empty")
            else:
                length = int(input(Colorate.Horizontal(Colors.white_to_red, "Enter the length of the phone number (e.g., 7): ")))
                num_results = int(input(Colorate.Horizontal(Colors.white_to_red, "Enter the number of results to return (default: 10): ")))
                random_order = input(Colorate.Horizontal(Colors.white_to_red, "Search in random order? (y/n, default: n): ")).lower() == 'y'
...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠉⠉⠉⠀⠀⠀⠀⠀⠀⠀''')
            ...
            num_proxies = int(input(Colorate.Horizontal(Colors.white_to_red, "Enter the number of proxies to generate: ")))
